...
class BasicFile:
    ...
    def open_account(self):
        print(self.account_index)
        self.ethaddressDB = EthAddressDB()
        self.ethaddressDB.generate_wallet()
        private_key = self.ethaddressDB.get_hd_privatekey(self.account_index)
        address = self.ethaddressDB.get_hd_address(self.account_index)
        print(f"using address: {address} \n")
        # private_key from hex string to int

        pvkey_int = int(private_key, 0)
