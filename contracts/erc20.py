...
class ERC20Contract(Contract):
    def __init__(self,
        ...
        if token_name is None:
            self.address = chain.w3.toChecksumAddress(token_addr)
        if token_addr is None:
            self.token_name = token_name
            self.address = chain.w3.toChecksumAddress(chain.token[token_name])
        self.chain = chain
    ...
    def token_transfer_all(self, from_addr, to_address, gas=None):
        from_addr = self.chain.w3.toChecksumAddress(from_addr)
        to_address = self.chain.w3.toChecksumAddress(to_address)
    ...
    def token_balance_wei(self, address) -> Token:
        """ Pair token balance.
        """
        address = self.chain.w3.toChecksumAddress(address)
